...
const elems: { [key: string]: { abbrev: string, proton: string, group: string, name: string } } = {
...
const anions: { [key: string]: { sum: any[], charge: number } } = {
...
function getOxc(name: string, endings: string[]) {
    var oxc = 0;
    var maxMatch = 0;
...
function getElem(name: string) {
    ...
    if (element == "") throw Error("Unknown element");

    return element;
}

function reduce(a: number, b: number) {
    var gcd = (a: number, b: number): number => {
...
function saltAnion(name: string) {
    var hydrogen = 0;
